...
<script>
import { listRole, getRole, delRole, addRole, updateRole, changeRoleStatus, menuTree, roleMenuTreeselect, saveRoleMenus } from "@/api/system/role"
  ...
  methods: {
    ...
    getRoleMenuTreeselect(roleId) {
      return roleMenuTreeselect(roleId).then(response => {
        return { checkedKeys: response.data || [] }
      })
    },
    /** 后端菜单树 → 树控件选项（id/label/children）。 */
    toTreeOptions(menus) {
      return (menus || []).map(m => ({
        id: m.menuId,
        label: m.menuName,
        children: this.toTreeOptions(m.children)
      }))
    },
    // 角色状态修改
        ...
        if (valid) {
          if (this.form.roleId != undefined) {
            ...
            updateRole(this.form).then(() => {
              return this.saveRoleMenusIfNeeded(this.form.roleId, menuIds)
            }).then(() => {
            ...
            addRole(this.form).then(res => {
              return this.saveRoleMenusIfNeeded(res.data.roleId, menuIds)
            }).then(() => {
     ...
     * @return Promise
     */
    saveRoleMenusIfNeeded(roleId, menuIds) {
