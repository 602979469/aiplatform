...
/**
 * 定时任务 Mapper。SQL 全部在 resources/mapper/SysJobMapper.xml 中。
 */
@Mapper
public interface SysJobMapper {
     ...
     * @return 当前页数据
     */
    List<SysJobDO> selectPage(SysJobQueryParam query);
     ...
     * @return 全量数据
     */
    List<SysJobDO> selectList(SysJobQueryParam query);
     ...
     * @return 总条数
     */
    long countByQuery(SysJobQueryParam query);

    /**
     * 新增，返回受影响行数；自增主键回填到 {@code sysJobDO.id}。
     *
     * @param sysJobDO 数据对象
     * @return 受影响行数
     */
    int insert(SysJobDO sysJobDO);
