...
/**
 * 通知公告 Mapper。SQL 全部在 resources/mapper/SysNoticeMapper.xml 中。
 */
@Mapper
public interface SysNoticeMapper {
     ...
     * @return 当前页数据
     */
    List<SysNoticeDO> selectPage(SysNoticeQueryParam query);
     ...
     * @return 全量数据
     */
    List<SysNoticeDO> selectList(SysNoticeQueryParam query);
     ...
     * @return 总条数
     */
    long countByQuery(SysNoticeQueryParam query);

    /**
     * 新增，返回受影响行数；自增主键回填到 {@code sysNoticeDO.id}。
     *
     * @param sysNoticeDO 数据对象
     * @return 受影响行数
     */
    int insert(SysNoticeDO sysNoticeDO);
