...
/**
 * 参数配置 Mapper。SQL 全部在 resources/mapper/SysConfigMapper.xml 中。
 */
@Mapper
public interface SysConfigMapper {
     ...
     * @return 当前页数据
     */
    List<SysConfigDO> selectPage(SysConfigQueryParam query);
     ...
     * @return 全量数据
     */
    List<SysConfigDO> selectList(SysConfigQueryParam query);
     ...
     * @return 总条数
     */
    long countByQuery(SysConfigQueryParam query);

    /**
     * 新增，返回受影响行数；自增主键回填到 {@code sysConfigDO.id}。
     *
     * @param sysConfigDO 数据对象
     * @return 受影响行数
     */
    int insert(SysConfigDO sysConfigDO);
     ...
     *
     * @param sysConfigDO 数据对象（至少含主键）
     * @return 受影响行数
     */
    int updateByCondition(SysConfigDO sysConfigDO);
