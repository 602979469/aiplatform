...
/**
 * 试卷模板知识点规则仓储：封装 Mapper，对外只暴露领域模型，不暴露 DO/DalQuery/DalResult。
 */
public interface KbExamTemplateRuleRepository {
     ...
     */
    KbExamTemplateRule findById(Long id);
     ...
     *
     * @param query 查询参数
     * @return 试卷模板知识点规则领域模型；未查询到返回 null，多条由 Mapper selectOne 抛 TooManyResultsException
     */
    KbExamTemplateRule findOne(KbExamTemplateRuleQueryParam query);

    /**
     * 新增。
     *
     * @param kbExamTemplateRule 试卷模板知识点规则
     * @return 新增后的试卷模板知识点规则；主键已回填到入参，返回同一对象
     */
    KbExamTemplateRule insert(KbExamTemplateRule kbExamTemplateRule);
     ...
     */
    int update(KbExamTemplateRule kbExamTemplateRule);
