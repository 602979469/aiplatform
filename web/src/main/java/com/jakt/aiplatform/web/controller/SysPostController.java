package com.jakt.aiplatform.web.controller;

import cn.hutool.core.util.ObjectUtil;
import com.jakt.aiplatform.biz.service.SysPostManager;
import com.jakt.aiplatform.web.assembler.SysPostAssembler;
import com.jakt.aiplatform.web.checker.SysPostParamChecker;
import com.jakt.aiplatform.web.param.SysPostCreateRequest;
import com.jakt.aiplatform.web.param.SysPostQueryRequest;
import com.jakt.aiplatform.web.param.SysPostUpdateRequest;
import com.jakt.aiplatform.web.result.SysPostResponse;
import com.jakt.aiplatform.web.result.AiPlatformResult;
import com.jakt.aiplatform.web.template.AiPlatformTemplate;
import com.jakt.aiplatform.common.util.tools.AiPlatformInvoker;
import com.jakt.aiplatform.core.model.domain.SysPost;
import com.jakt.aiplatform.core.model.enums.ErrorCodeEnum;
import com.jakt.aiplatform.core.model.result.PageResult;
import io.swagger.v3.oas.annotations.tags.Tag;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * 岗位管理接口。Controller 只做参数校验、DTO 转换与结果包装，不含业务规则；
 * 参数校验、异常封装、请求日志与 Result 组装统一交给 AiPlatformTemplate。
 */
@RestController
@RequestMapping("/api/v1/sysPosts")
@Tag(name = "岗位管理")
public class SysPostController {

    /** 岗位 Manager。 */
    private final SysPostManager sysPostManager;

    public SysPostController(SysPostManager sysPostManager) {
        this.sysPostManager = sysPostManager;
    }

    /**
     * 创建岗位。
     *
     * @param request 创建岗位请求体
     * @return 创建成功后的岗位信息
     */
    @PostMapping
    public AiPlatformResult<SysPostResponse> create(@RequestBody SysPostCreateRequest request) {
        return AiPlatformTemplate.execute(request, new AiPlatformTemplate.Callback<>() {

            @Override
            public void beforeService(SysPostCreateRequest param) {
                SysPostParamChecker.checkSysPostCreateRequest(param);
            }

            @Override
            public SysPostResponse execute(SysPostCreateRequest param) {
                // TODO 接入 SysPostManager（RuoYi 移植过渡）
                return null;
            }

            @Override
            public void afterService(SysPostCreateRequest param, SysPostResponse result) {
            }
        });
    }

    /**
     * 按 ID 查询岗位。
     *
     * @param id 岗位 ID
     * @return 岗位信息
     */
    @GetMapping("/{id}")
    public AiPlatformResult<SysPostResponse> get(@PathVariable Long id) {
        return AiPlatformTemplate.execute(id, new AiPlatformTemplate.Callback<>() {

            @Override
            public void beforeService(Long param) {
                SysPostParamChecker.checkId(param);
            }

            @Override
            public SysPostResponse execute(Long param) {
                // TODO 接入 SysPostManager（RuoYi 移植过渡）
                return null;
            }

            @Override
            public void afterService(Long param, SysPostResponse result) {
            }
        });
    }

    /**
     * 分页查询岗位。
     *
     * @param request 查询条件（含分页参数与时间区间）
     * @return 分页结果
     */
    @GetMapping("/page")
    public AiPlatformResult<PageResult<SysPostResponse>> page(SysPostQueryRequest request) {
        
        return AiPlatformTemplate.execute(request, new AiPlatformTemplate.Callback<>() {

            @Override
            public void beforeService(SysPostQueryRequest param) {
                SysPostParamChecker.checkSysPostQueryRequest(param);
            }

            @Override
            public PageResult<SysPostResponse> execute(SysPostQueryRequest param) {
                // TODO 接入 SysPostManager（RuoYi 移植过渡）
                return null;
            }
        });
    }

    /**
     * 更新岗位（全量）。
     * 注意：PUT 为全量覆盖，未传字段会被置 NULL；部分更新请走 updateByCondition（Manager/DomainService）。
     *
     * @param id      岗位 ID
     * @param request 更新内容
     * @return 更新后的岗位信息
     */
    @PutMapping("/{id}")
    public AiPlatformResult<Void> update(@PathVariable Long id, @RequestBody SysPostUpdateRequest request) {
        return AiPlatformTemplate.executeWithoutResult(request, new AiPlatformTemplate.CallbackWithoutResult<>() {

            @Override
            public void beforeService(SysPostUpdateRequest param) {
                SysPostParamChecker.checkId(id);
                SysPostParamChecker.checkSysPostUpdateRequest(param);
            }

            @Override
            public void execute(SysPostUpdateRequest param) {
                // TODO 接入 SysPostManager（RuoYi 移植过渡）
            }
        });
    }

    /**
     * 删除岗位。
     *
     * @param id 岗位 ID
     * @return 删除结果
     */
    @DeleteMapping("/{id}")
    public AiPlatformResult<Void> delete(@PathVariable Long id) {
        return AiPlatformTemplate.executeWithoutResult(id, new AiPlatformTemplate.CallbackWithoutResult<>() {

            @Override
            public void beforeService(Long id) {
                SysPostParamChecker.checkId(id);
            }

            @Override
            public void execute(Long id) {
                // TODO 接入 SysPostManager（RuoYi 移植过渡）
            }
        });
    }
}
