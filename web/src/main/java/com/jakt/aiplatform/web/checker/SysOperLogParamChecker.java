package com.jakt.aiplatform.web.checker;

import cn.hutool.core.util.ObjectUtil;
import com.jakt.aiplatform.common.util.tools.AiPlatformInvoker;
import com.jakt.aiplatform.common.util.tools.AiPlatformParamValidator;
import com.jakt.aiplatform.core.model.enums.ErrorCodeEnum;
import com.jakt.aiplatform.web.param.SysOperLogCreateRequest;
import com.jakt.aiplatform.web.param.SysOperLogQueryRequest;
import com.jakt.aiplatform.web.param.SysOperLogUpdateRequest;

/**
 * 操作日志参数检查器
 */
public class SysOperLogParamChecker {

    private SysOperLogParamChecker() {
    }

    /**
     * 检查操作日志创建参数。
     *
     * @param request 操作日志创建请求
     */
    public static void checkSysOperLogCreateRequest(SysOperLogCreateRequest request) {
        AiPlatformInvoker.throwErrWhenNull(request, ErrorCodeEnum.PARAM_INVALID, "创建参数不能为空");
        AiPlatformParamValidator.validate(request);
    }

    /**
     * 检查操作日志更新参数。
     *
     * @param request 操作日志更新请求
     */
    public static void checkSysOperLogUpdateRequest(SysOperLogUpdateRequest request) {
        AiPlatformInvoker.throwErrWhenNull(request, ErrorCodeEnum.PARAM_INVALID, "更新参数不能为空");
        AiPlatformParamValidator.validate(request);
    }

    /**
     * 检查操作日志 ID 参数（按 ID 查询/删除共用）。
     *
     * @param id 操作日志 ID
     */
    public static void checkId(Long id) {
        AiPlatformInvoker.throwErrWhenNull(id, ErrorCodeEnum.PARAM_INVALID, "操作日志ID不能为空");
    }

    /**
     * 检查操作日志查询参数
     *
     * @param request 操作日志查询请求，可为 null
     */
    public static void checkSysOperLogQueryRequest(SysOperLogQueryRequest request) {
        if (ObjectUtil.isNull(request)) {
            return;
        }
        AiPlatformParamValidator.validate(request);
    }
}
